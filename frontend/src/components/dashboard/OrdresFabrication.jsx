//OrdresFabrication.jsx
import React, { useState, useEffect } from "react";
import axios from "axios";

const BASE_URL = "http://127.0.0.1:8000/api/ordres-fabrication";
...
const OrdresFabrication = () => {
  ...
  // Format : OF-YYYY-NNN (ex: OF-2026-001)
  // ─────────────────────────────────────────────
  const generateNextOFNumber = (existingOfs) => {
    const year = new Date().getFullYear();
    const prefix = `OF-${year}-`;

    const currentYearNumbers = existingOfs
      .map(o => o.numero)
    ...
    const nextNumber =
      currentYearNumbers.length > 0
        ? Math.max(...currentYearNumbers) + 1
        : 1;

    // Formatage sur 3 chiffres : 001, 002, ...
  ...
  const fetchOF = async () => {
    try {
      ...
      // ✅ Générer le premier numéro disponible dès le chargement
      setFormOF(prev => ({
        ...prev,
        numero: generateNextOFNumber(ofsWithStatus)
      }));
    } catch (err) {
      console.error(err);
      // Même en cas d'erreur réseau, générer un numéro de base
      setFormOF(prev => ({
        ...prev,
        numero: generateNextOFNumber([])
      }));
    }
  };
  ...
  const handleCreateOF = async (e) => {
    ...
    try {
      ...
      if (res.status === 200 || res.status === 201) {
        ...
        const updatedOfs = [...ofs, newOF];
        setOfs(updatedOfs);

        // ✅ Réinitialiser le formulaire avec le PROCHAIN numéro auto-généré
        setFormOF({
          numero: generateNextOFNumber(updatedOfs),
          machine: "",
          produit: "",
          quantite: "",
          date_debut: "",
  ...
  // Réinitialisation : conserve le numéro auto-généré
  const handleReset = () => {
    setFormOF({
      numero: generateNextOFNumber(ofs),
      machine: "",
      produit: "",
      quantite: "",
      date_debut: "",
  ...
  const styles = {
    ...
    // ── Style spécial : champ numéro auto-généré ──
    inputReadOnly: {
      padding: "0.75rem 1rem",
          ...
          <div style={styles.formCard}>
            ...
            <form onSubmit={handleCreateOF} style={styles.form}>
              <div style={styles.formGrid}>
                ...
                {/* ── Numéro OF auto-généré (lecture seule) ── */}
                <div style={styles.formGroup}>
                  ...
                  <input
                    ...
                    tabIndex={-1}
                  />
                  <span style={styles.autoGeneratedBadge}>
